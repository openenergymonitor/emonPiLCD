...
config.read(
    path + "/emonPiLCD.cfg"
)  # FIXME should live in /etc, not /usr/share/emonPiLCD
...
# push button GPIO
push_button_gpio = config.getint("general", "push_button_gpio")
...
def main():
    ...
    for i2c_address in lcd_i2c:
        ...
        print(
            "I2C LCD NOT DETECTED on either 0x"
            + str(lcd_i2c)
            + " ...exiting LCD script"
        )
        logger.error(
    ...
    try:
        push_btn = Button(push_button_gpio, pull_up=True, bounce_time=0.1, hold_time=5)
